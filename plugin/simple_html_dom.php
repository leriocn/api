<?php
/*Demo
$result = ExtendManager_Class::getContent($url);
//ExtendManager_Class::writeLogToFile($result);
echo $result;
$resultData = array();
...
try
{
    $arrTr = $html->getElementById('table')->children(1)->find('tr');

    $i = 0;
    foreach($arrTr as $tr) 
    {
        if($i>=$this->_param->top)
        {
            break;
        }
